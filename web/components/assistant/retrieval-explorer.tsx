...
// Which real capability each retrieval source belongs to — groups the flat
// trace list under the same CRM/Knowledge headers the rest of the UI uses
// (see TOOL_LABEL in message.tsx), so "what grounded this" reads as
// capability lanes rather than raw source-string plumbing.
  ...
  return (
    <details open={open} className="w-full max-w-[88%] rounded-lg border border-border bg-background text-[12px]">
      ...
      <div className="space-y-3 px-3 pb-3">
        {groupTraces(traces).map(([group, groupTraces]) => (
          <div key={group} className="space-y-2">
            {group !== "_ungrouped" && (
              <div className="text-[10.5px] font-semibold uppercase tracking-wide text-muted-foreground/70">
                {lang === "ja" ? GROUP_META[group]?.ja ?? group : GROUP_META[group]?.en ?? group}
              </div>
            )}
            {groupTraces.map((tr, i) => {
...
// Bucket traces by capability, preserving first-seen order; sources with no
// known capability mapping fall into an unlabeled "_ungrouped" bucket so
// nothing silently disappears from the explorer.
function groupTraces(traces: RetrievalTrace[]): [string, RetrievalTrace[]][] {
  const order: string[] = [];
  const buckets = new Map<string, RetrievalTrace[]>();
  for (const tr of traces) {
    const group = SOURCE_GROUP[tr.source] ?? "_ungrouped";
    if (!buckets.has(group)) { buckets.set(group, []); order.push(group); }
    buckets.get(group)!.push(tr);
  }
